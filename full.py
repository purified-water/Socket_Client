...
def start_connections(host, port, num_conns, urlInput):
    ...
    for i in range(0, num_conns):
        ...
        data = types.SimpleNamespace(
            connid = connid,
            msg_total = sum(len(m) for m in urlInput),
            recv_total = 0,
            messages = urlInput.copy(),
            outb = b"",
        )
